...
pub(crate) use cmds::{
    cloud::{aws_cmd, container, curl_cmd, psql_cmd, wget_cmd},
    dotnet::{binlog, dotnet_cmd, dotnet_format_report, dotnet_trx},
    git::{diff_cmd, gh_cmd, git, gt_cmd},
    go::{go_cmd, golangci_cmd},
    js::{
        lint_cmd, next_cmd, npm_cmd, playwright_cmd, pnpm_cmd, prettier_cmd, prisma_cmd, tsc_cmd,
        vitest_cmd,
    },
    python::{mypy_cmd, pip_cmd, pytest_cmd, ruff_cmd},
    ruby::{rake_cmd, rspec_cmd, rubocop_cmd},
    rust::{cargo_cmd, runner},
    system::{
        deps, env_cmd, find_cmd, format_cmd, grep_cmd, json_cmd, local_llm, log_cmd, ls,
...
fn merge_pnpm_args(filters: &[String], args: &[String]) -> Vec<String> {
    dispatch::merge_pnpm_args(filters, args)
}
...
fn merge_pnpm_args_os(filters: &[String], args: &[OsString]) -> Vec<OsString> {
    dispatch::merge_pnpm_args_os(filters, args)
}
...
fn validate_pnpm_filters(filters: &[String], command: &PnpmCommands) -> Option<String> {
    dispatch::validate_pnpm_filters(filters, command)
}
